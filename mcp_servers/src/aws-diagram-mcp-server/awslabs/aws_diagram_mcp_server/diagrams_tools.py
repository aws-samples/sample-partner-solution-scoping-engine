...
from awslabs.aws_diagram_mcp_server.s3_manager import upload_diagram_to_s3


# Get logger from the root logger configuration
...
async def generate_diagram(
    ...
    # Always use 'diagram' as filename for consistent version control
    # Ignore any custom filename provided by the LLM
    filename = 'diagram'
    ...
    try:
        ...
        # nosec B102 - exec used for controlled import statements in diagram generation namespace
        exec(  # nosem: python.lang.security.audit.exec-detected.exec-detected # nosec B102 - exec used for controlled import statements in diagram generation namespace
            'import os',
            namespace,
        )
        # nosec B102 - These exec calls are necessary to import modules in the namespace
        exec(  # nosem: python.lang.security.audit.exec-detected.exec-detected # nosec B102 - These exec calls are necessary to import modules in the namespac
            'import diagrams', namespace
        )
        # nosec B102 - These exec calls are necessary to import modules in the namespace
        exec(  # nosem: python.lang.security.audit.exec-detected.exec-detected # nosec B102 - These exec calls are necessary to import modules in the namespace
...
# Import modules for namespace access (aws.storage.S3, gcp.compute.GCE, etc.)
from diagrams import aws
from diagrams import gcp
from diagrams import k8s
from diagrams import generic

# Import AWS classes directly for direct access (S3 syntax)
...
""",
        ...
        )  # nosem: python.lang.security.audit.exec-detected.exec-detected # nosec B102 - These exec calls are necessary to import modules in the namespace
        ...
        exec(code, namespace)  # nosem: python.lang.security.audit.exec-detected.exec-detected # nosec B102 - These exec calls are necessary to import modules in the namespace
        ...
        upload_to_s3 = os.getenv('UPLOAD_TO_S3', 'False')
        response = None
        if os.path.exists(png_path) and upload_to_s3 == 'True':

            logger.debug("Uploading file to s3")
            
            # Upload scripts to S3
            upload_result = await upload_diagram_to_s3(chat_id, png_path)
            ...
            response = DiagramGenerateResponse(
                status='success',                
                chat_id=chat_id,
                message=f'Diagram generated successfully to {upload_result["s3_url"]}',
                s3_bucket=upload_result["bucket"],
                s3_url=upload_result["s3_url"],
                s3_key=upload_result["s3_key"],
                s3_file_version_id=upload_result["version_id"],
                file_size=upload_result["file_size"],
                metadata_key=upload_result["metadata_key"],
                generation_time=round(generation_time, 2)
            )
            logger.debug(f'Returning object 1: {response}')
        else:
            generation_time = time.time() - start_time
            if not os.path.exists(png_path):
                response = DiagramGenerateResponse(
                    status='error',
                    message=f'Diagram code executed but no image was generated. The diagram code may contain invalid service class names or syntax errors that the diagrams library silently ignored.',
                    generation_time=round(generation_time, 2)
                )
            else:
                response = DiagramGenerateResponse(
                    status='success',                
                    chat_id=chat_id,
                    message=f'Diagram generated successfully and saved in {png_path}',
                    local_path=png_path,
                    generation_time=round(generation_time, 2)
                )
            logger.debug(f'Returning object 2: {response}')
