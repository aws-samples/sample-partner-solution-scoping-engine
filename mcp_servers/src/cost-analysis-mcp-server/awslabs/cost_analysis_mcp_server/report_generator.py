...
from awslabs.cost_analysis_mcp_server.s3_manager import upload_pricing_report_to_s3
...
async def _generate_custom_data_report(
    ...
    # Write to S3 if chat_id is provided, otherwise write to local file
    if chat_id:
        try:
            # Always use 'pricing' as filename for consistency
            filename = 'pricing'
            format_type = 'markdown'
            if output_file and output_file.endswith('.csv'):
                format_type = 'csv'
            
            upload_result = await upload_pricing_report_to_s3(
                chat_id=chat_id,
            ...
            # Add CloudFront link to the end of the report content
            if format_type == 'markdown':
                # Generate CloudFront URL using backend route pattern
                file_extension = 'md' if format_type == 'markdown' else format_type
                cloudfront_url = f"/api/chats/{chat_id}/documents/pricing/cf_signedurl/pricing.{file_extension}"
                report += f"\n\n---\n**📊 [View this cost analysis report]({cloudfront_url})**\n"
            
            if ctx:
                await ctx.info(f'Report saved to S3: {upload_result["s3_url"]}')
            
            # Save cost data to DynamoDB
            try:
                await _save_cost_data_to_dynamodb(
                    chat_id=chat_id,
                ...
                if ctx:
                    await ctx.error(f'Failed to save cost data to DynamoDB: {e}')
        except Exception as e:
            if ctx:
                await ctx.error(f'Failed to upload report to S3: {e}')
    elif output_file:
        try:
    ...
    # If we uploaded to S3, return structured file metadata like diagram server
    if chat_id and upload_result:
        # Create structured response similar to DiagramGenerateResponse
        response_data = {
            "status": "success",
            "chat_id": chat_id,
            "message": f"Cost analysis report generated successfully",
            "s3_bucket": upload_result.get("bucket"),
            "s3_url": upload_result.get("s3_url"),
            "s3_key": upload_result.get("s3_key"),
            "s3_file_version_id": upload_result.get("version_id"),
            "file_size": upload_result.get("file_size"),
            "metadata_key": upload_result.get("metadata_key"),
            "report_content": report
        }
        import json
...
async def _generate_pricing_data_report(
    ...
    if chat_id:
        try:
            ...
            # Save cost data to DynamoDB - extract base cost from cost_tables
            try:
                # Try to extract base cost from projected costs table or cost calculation
                base_cost = None
                if 'projected_costs_table' in cost_tables:
                    # Extract base cost from projected costs table
                    base_cost = _extract_base_cost_from_table(cost_tables['projected_costs_table'])
...
async def _generate_csv_report(
    ...
    if chat_id:
        try:
            ...
            if ctx:
                await ctx.info(f'CSV report saved to S3: {upload_result["s3_url"]}')
        except Exception as e:
            if ctx:
                await ctx.error(f'Failed to upload CSV report to S3: {e}')
    elif output_file:
        try:
            output_path = Path(output_file)
            output_path.parent.mkdir(parents=True, exist_ok=True)
...
async def generate_cost_report(
    ...
    output_file: Optional[str] = None,
    chat_id: Optional[str] = None,  # Chat ID for S3 storage
    ...
    try:
        ...
        if format == 'csv':
            ...
            return await _generate_csv_report(cost_data, output_file, chat_id, ctx)
        else:
            ...
            if 'services' in cost_data:
                ...
                return await _generate_custom_data_report(cost_data, output_file, chat_id, ctx)
            else:
...
async def _save_cost_data_to_dynamodb(
    chat_id: str,
    estimated_monthly_cost: Optional[float],
    s3_key: Optional[str],
    ctx: Optional[Context] = None
) -> None:
    """Save cost analysis data to DynamoDB."""
    logger.info(f'Starting DynamoDB save for chat_id={chat_id}, cost={estimated_monthly_cost}')
    
    if not chat_id or estimated_monthly_cost is None:
        logger.warning(f'Skipping DynamoDB save - chat_id={chat_id}, cost={estimated_monthly_cost}')
        return
    
    try:
        # Import and use the config system
        from backend.config.app_config import CustomerConfig
        table_name = CustomerConfig.get_value('DYNAMODB_TABLE_NAME')
        
        if not table_name:
            logger.error('DYNAMODB_TABLE_NAME not configured in CustomerConfig')
            return
        ...
        # Prepare cost data
        cost_data = {
            'estimated_monthly_cost': {'N': str(estimated_monthly_cost)},
            'cost_analysis_timestamp': {'S': datetime.now().isoformat()},
        }
        
        if s3_key:
            cost_data['cost_report_s3_key'] = {'S': s3_key}
        
        logger.info(f'Prepared cost_data: {cost_data}')
        
        # First find the chat by chat_id using GSI
        logger.info(f'Querying GSI for chat_id: {chat_id}')
        
        gsi_response = dynamodb.query(
        ...
        logger.info(f'GSI query response: {len(gsi_response.get("Items", []))} items found')
        
        if not gsi_response['Items']:
            logger.error(f'Chat with ID {chat_id} not found in DynamoDB')
            return
        ...
        # Update the chat record with cost data using correct primary key
        update_expression = 'SET ' + ', '.join([f'{k} = :{k}' for k in cost_data.keys()])
        expression_attribute_values = {f':{k}': v for k, v in cost_data.items()}
        ...
        logger.info(f'DynamoDB update response: {response.get("Attributes", {})}')
        logger.success(f'Cost data saved to DynamoDB for chat_id: {chat_id}')
            
    except Exception as e:
        logger.error(f'Failed to save cost data to DynamoDB: {str(e)}')
        if ctx:
            await ctx.error(f'Failed to save cost data to DynamoDB: {str(e)}')
        raise


def _extract_base_cost_from_table(table_content: str) -> Optional[float]:
    """Extract base monthly cost from projected costs table."""
    try:
        # Look for patterns like "$127.50" or "$1,234.56" in the table
        cost_pattern = r'\$([0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]{2})?)'
        matches = re.findall(cost_pattern, table_content)
        
        if matches:
            # Get the first cost value (usually the base cost)
