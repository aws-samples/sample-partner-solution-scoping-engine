...
try:
    ...
    logger = setup_mcp_logging("cost-analysis-mcp-server")
    logger.info("Cost Analysis MCP Server logger initialized using backend configuration")
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            RotatingFileHandler("cost-analysis-mcp-server.log", maxBytes=10*1024*1024, backupCount=5)
        ]
    )
    logger = logging.getLogger("cost-analysis-mcp-server")
...
class PricingFilter(BaseModel):
    ...
    type: str = Field('TERM_MATCH', alias='Type', description='The type of filter match')
...
async def get_pricing_from_web(service_code: str, ctx: Context) -> Optional[Dict]:
    ...
    try:
        ...
        async with AsyncClient() as client:
            ...
            for unwanted in soup(['nav', 'header', 'footer']):
                unwanted.decompose()
            
            # Remove elements with common navigation/footer classes and IDs
            unwanted_selectors = [
                '.aws-header', '.aws-footer', '.aws-nav', 
                '#aws-header', '#aws-footer', '#aws-nav',
...
- ALWAYS remember {{estimated_monthly_solution_cost}} which is the total of all individual service total costs. 
- ALWAYS remember {{estimated_annual_solution_cost}} which equals estimated_monthly_solution_cost*12
...
```json
{
  ...
  "chat_id": "12345",  // Chat ID for S3 storage - reports will be saved to S3 bucket at {chat_id}/pricing/{filename}
...
async def generate_cost_report_wrapper(
    ...
    logger.debug(f"Generate cost report request - service: {service_name}, chat_id: {chat_id}")
    logger.debug(f"Format: {format}, pricing_model: {pricing_model}")
    logger.debug(f"Related services: {len(related_services) if related_services else 0}, has detailed_cost_data: {detailed_cost_data is not None}")

    logger.info(f'Starting cost analysis report generation for service: {service_name}, chat_id: {chat_id}')
...
def main():
    ...
    logger.info("Starting Cost Analysis MCP Server")
